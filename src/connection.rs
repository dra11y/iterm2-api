...
impl ITerm2Connection {
    pub async fn connect() -> Result<Self> {
        ...
        if !socket_path.exists() {
            return Err(Error::Connection(
                format!("iTerm2 Unix domain socket not found at: {}. iTerm2 must be running with API server enabled.", 
                socket_path.display())
            ));
        }
        
        let stream = UnixStream::connect(&socket_path).await
            .map_err(|e| Error::Connection(format!("Failed to connect to Unix domain socket: {}", e)))?;
        ...
        let request = Request::builder()
            ...
            .header("x-iterm2-library-version", "rust 1.0") // Required header
            .body(())
        ...
        let request = if let Ok(cookie) = std::env::var("ITERM2_COOKIE") {
            Request::builder()
                ...
                .body(())
                .map_err(|e| Error::Connection(format!("Failed to build WebSocket request with cookie: {}", e)))?
        } else {
            request
        };
    ...
    pub async fn create_tab(&mut self, profile_name: Option<&str>) -> Result<SessionSummary> {
        ...
        if response.has_create_tab_response() {
            ...
            if create_response.status() == create_tab_response::Status::OK {
                ...
                Err(Error::Api(format!(
                    "Create tab failed: {:?}", create_response.status()
                )))
            }
        } else {
    ...
    pub async fn send_text(&mut self, session_id: &str, text: &str) -> Result<()> {
        ...
        if response.has_send_text_response() {
            ...
            if send_response.status() == send_text_response::Status::OK {
                ...
                Err(Error::Api(format!(
                    "Send text failed: {:?}", send_response.status()
                )))
            }
        } else {
